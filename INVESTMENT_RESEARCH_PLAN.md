...
**Progress — 2026-09-10:** stage 0 and the Stage 1 software foundation are complete. Stage 2's BTC core, signal-study primitive and venue/capital context are implemented: a quota-controlled Coin Metrics daily batch, long-term uPlot charts, daily/weekly indicators, confirmed regimes, valuation and drawdown/cycle comparisons, immutable forward-return studies, native BTC OI/funding evidence, and covered USD-pegged stablecoin supply with exact 30D changes. The worker has 124 scheduled jobs and 22 normalized series; empty enrichment slots skip without consuming quota or claiming coverage. Live BTC price coverage is 2010-07-18–2026-09-09 (5,898 values); MVRV, market cap and supply currently end a day earlier and degrade individually. Capital history spans 2017-11-29–2026-09-09 (3,207 values), with 337 USD-pegged assets in the separately archived current catalog; historical constituents remain unavailable. See [BTC core evidence](docs/data/stage2-btc-2026-09-09.json), [context evidence](docs/data/stage2-context-2026-09-09.json) and [methods](docs/data/BTC_RESEARCH.md). **The remaining Stage 1 operational condition is an always-on host with sustained coverage; local supervision cannot guarantee uptime while this Mac sleeps or Docker is stopped.** Local worker/backup supervision and restore verification remain in place. Stage 3 adds the composed Market Overview: provider-global aggregates with declared scope, sampled hourly market-cap/volume/dominance charts, tracked-page breadth, the share of covered assets above their own 50D/200D averages, the BTC regime summary, covered stablecoin liquidity, BTC-relative 7D/30D/90D performance, notable changes, and sector leadership held at its curated-classification gate. On 2026-09-10 the live page covered 100 tracked assets (27 up / 70 down / 2 unchanged / 1 unknown), 4 of 100 assets with 200 contiguous daily samples, a bullish confirmed BTC regime and $310.4B covered USD-pegged supply (+1.42% over 30D); provider-global volume changes stayed unavailable because hourly sampling had begun that morning. See [Market Overview methods](docs/data/MARKET_OVERVIEW.md) and [dated evidence](docs/data/stage3-overview-2026-09-10.json). Stage 4 adds the perp discovery workspace: Perp DEX Projects and New Perp Listings views over the archives that have been running since stage 1, one new accessible DefiLlama covered-open-interest feed inside the slot the stage 0 budget already reserved, curated immutable protocol/venue and instrument/underlying links, and lifecycle alerts derived from the immutable event log. On 2026-09-10 the live workspace held 471 perp-universe protocols (446 in the provider `Derivatives` category, 127 in the covered open-interest set, 103 with a reported value), a covered aggregate of $25.37B on 2026-09-09 with 2,023 daily values from 2021-02-25, and all 517 archived Hyperliquid markets still at their archive baseline — so zero new listings, zero alerts, 201 markets already carrying the venue delisting flag, four verified crypto underlyings and one archived order book. Perp volume and market share stay gated at HTTP 402, and no free feed publishes protocol or token launch dates, so every project entry is undated and the 180-day early filter selects nothing and says so. See [perp methods](docs/data/PERP_DISCOVERY.md) and [dated evidence](docs/data/stage4-perp-2026-09-10.json). Stage 5 adds the Altcoin Discovery workspace with no new acquisition feed: Emerging Projects compose the archived tracked page with the four priority daily price/volume series (BTC-relative 7D/30D/90D returns, 30D/90D realized volatility, maximum drawdown and recovery, BTC correlation and beta, a 30D volume median and 7-over-30 acceleration, supply/FDV dilution), each research default reported as pass/fail/unknown with an unknown check never a pass, curated immutable stablecoin/tokenized-fund/commodity/wrapped-duplicate exclusions with peg-symbol collisions only flagged, and the 40/30/30 Attention percentile kept out of prime space with its unvalidated-v0 caveat; Spot Launches compose the Solana/Base new-pool samples with DEX Screener pair/promotion enrichment, deduplicating by chain and contract across every pool ever sampled so a later pool never makes an old token new, keeping unknown pool liquidity unknown and off the shortlist, and labelling paid boosts as promotion. Migration 0011 adds only curated `asset_universe_exclusions` and `spot_quote_tokens` reference tables. On 2026-09-10 the live workspace covered 100 tracked assets with 0 eligible (only 4 carry archived daily history and all sit outside the $10M–$2B band), 26 curated exclusions and 3 peg-symbol collisions flagged for review; Spot Launches held 5,279 tokens observed across ~2 days of sampling (3,633 Solana, 1,646 Base), 93 shortlisted, 626 with unknown liquidity and 254 multi-pool tokens, replay stable and point-in-time. Verified asset launch dates, the 1,000-asset expansion, token value capture and sector-peer comparison stay visibly gated. See [altcoin methods](docs/data/ALTCOIN_DISCOVERY.md) and [dated evidence](docs/data/stage5-altcoin-2026-09-10.json). Next: Stage 6 Backtest Lab and expanded evidence. Manual backfills do not start replay coverage; the 1,000-asset expansion, macro and unsupported feeds remain gated.
...
Validation passes 165 checks: 86 API unit cases, 52 PostgreSQL integration cases, twelve capability/budget cases and fifteen browser scenarios. API tests discover files recursively and assert minimum executed case counts with no skips; database and browser tests use isolated schemas. Coverage includes migrations, immutable revisions/replay, shared weighted quotas including manual/probe requests, lease/retry recovery, lifecycle and gap semantics, scope/null handling, persistent research state, note conflicts, threshold crossings with immutable evidence, and stored-only API reads. Stage 2 adds regime confirmation and gap resets, completed weekly calculations, exact calendar outcomes, BTC-relative compounding, missing/immature horizons, immutable study reproducibility, chart controls/exports and reopening saved studies. Context checks cover venue identity/units, separate signed funding rates, independent feed failures, exact 30D supply endpoints, changing peg coverage, aggregate scope, null revisions, quota use and replay stability. Stage 3 adds contiguous-window trailing averages, exact calendar and compounded BTC-relative returns, sampled-comparison tolerances, uncovered-versus-below participation, per-block degradation and per-feed replay refusal, leader/laggard separation, gated sector coverage and composed signal changes. Stage 4 adds covered open-interest parsing with live-day exclusion and irregular-history rejection, trailing-window gaps, the covered-universe denominator, provider-category universe composition, undated first-observed ordering, baseline-versus-listing classification, catalog absence, namespace collisions, unresolved underlyings, order-book versus impact-only liquidity, spot separated from perpetual profit and loss, derived lifecycle alerts with separate acknowledgement, and perp route validation. Stage 5 adds contiguous trailing windows and medians, 7-over-30 volume acceleration, cross-sectional percentiles with tie splitting, gap-safe daily log returns, realized volatility and maximum drawdown with open-drawdown handling, shared-day correlation and OLS beta, pass/fail/unknown eligibility separation, curated identity exclusions kept apart from peg-symbol collisions, attention ranking confined to the eligible universe, token-level pool deduplication with first-observation stability, unknown-liquidity handling, membership-safe liquidity change, promotion labelling, unknown-only risk status, and altcoin route validation. Full strategy-backtest invariants remain future coverage.
...
| Altcoin Discovery | Find evidence-backed candidates | Implemented: Perp DEX Projects and New Perp Listings (stage 4); Emerging Projects over the archived tracked page with BTC-relative returns, realized volatility, drawdown, correlation/beta, volume screens and dilution, curated identity exclusions and a de-emphasised Attention placeholder; general Solana/Base spot-token launches with token-level dedup, unknown-liquidity handling, promotion labelling and unknown-only risk status. Historical outcomes stay gated on replay coverage; the 1,000-asset universe, verified launch dates, token value capture and sector-peer comparison remain gated |
...
- [x] Stage 4 perp discovery: migration 0010; one new daily DefiLlama covered-open-interest feed with an immutable covered-universe aggregate series inside the reserved budget slot; curated immutable protocol/venue and instrument/underlying links; composed `GET /api/v1/perp/projects` (`perp-projects:v1`) and `GET /api/v1/perp/listings` (`perp-listings:v1`) reading only stored archives, with per-feed replay refusal, declared universe scope, recomputed open-interest changes beside the provider percentages, trailing 30D fundamentals for individually verified protocols, separate protocol/token/instrument/underlying identity, baseline-versus-listing classification, retained catalog absences, native open-interest units, funding intervals, order-book liquidity where archived, spot-versus-BTC returns kept apart from perpetual profit and loss, and lifecycle alerts derived from the immutable event log with separate read acknowledgement. The `#perp` page opens formula, scope, freshness and provenance for every figure and keeps volume, launch dates and token terms visibly gated.
- [x] Stage 4 verification: build, 68 unit/capability cases, 44 isolated PostgreSQL cases and thirteen browser scenarios pass. Local migration 0010 applied; supervised worker restarted; the first metered covered-open-interest acquisition archived 127 protocols and 2,023 completed daily aggregate values. Live acceptance returned HTTP 200 on all four routes, refused an uncovered 2017 cutoff with HTTP 409 and served a covered cutoff as point-in-time. [Dated perp evidence](docs/data/stage4-perp-2026-09-10.json).
- [x] Stage 5 broader altcoin research: migration 0011 adds only curated immutable `asset_universe_exclusions` and `spot_quote_tokens` reference tables; no new acquisition feed. Composed `GET /api/v1/altcoin/emerging` (`emerging-projects:v1`) and `GET /api/v1/altcoin/launches` (`spot-launches:v1`) read only stored archives with per-feed replay refusal. Emerging Projects screen the archived tracked page on editable research defaults (market-cap band, ≥90 contiguous archived daily samples, ≥$1M 30D volume median, no curated identity exclusion), each reported pass/fail/unknown separately with an unknown check never a pass; archive-computed BTC-relative 7D/30D/90D returns, 30D/90D realized volatility, 90D maximum drawdown with recovery, BTC correlation and OLS beta over shared days, a 30D volume median and 7-over-30 acceleration, and supply/FDV dilution, all unavailable rather than approximate on an incomplete window. Stablecoins, tokenized funds, commodity-backed tokens and wrapped duplicates are excluded through curated rows; a DefiLlama peg-catalog symbol collision is flagged for review only and never excludes. The 40/30/30 Attention percentile is computed inside the eligible universe, left unranked on any missing input, and kept out of prime interface space with its unvalidated-v0 caveat. Spot Launches compose the Solana/Base new-pool samples with DEX Screener pair and promotion enrichment, deduplicating by chain and base-token contract across every pool ever sampled so a later pool never resets a token's first observation, summing observed pool liquidity with unknown values kept unknown and off the default shortlist, reporting a liquidity change only when every contributing pool has an earlier observation and stating the actual elapsed hours, labelling paid boosts as promotion, and holding every contract-level risk check at unknown. Token value capture, sector-peer comparison, verified asset launch dates and the 1,000-asset expansion stay visibly gated.
- [x] Stage 5 verification: build, 86 unit/capability cases (18 new), 52 isolated PostgreSQL cases (8 new) and fifteen browser scenarios (2 new) pass. Local migration 0011 applied. Live acceptance on 2026-09-10 returned HTTP 200 on both routes, refused an uncovered 2017 cutoff with HTTP 409 and served a covered cutoff as point-in-time and stable. The live workspace showed 100 tracked assets with 0 eligible (4 carry archived daily history, all outside the $10M–$2B band), 26 curated exclusions and 3 peg-symbol collisions flagged; Spot Launches held 5,279 observed tokens (3,633 Solana / 1,646 Base) across ~2 days of sampling, 93 shortlisted, 626 with unknown liquidity, 254 multi-pool. [Dated altcoin evidence](docs/data/stage5-altcoin-2026-09-10.json). Two live-data defects found and fixed during the build: a bare ticker collision (`M`) had wrongly excluded a project, and the spot enrichment feed carries no token symbol so pool labels are shown as provider labels rather than verified symbols.
- [ ] Stage 6: Backtest Lab and expanded evidence — historical replay, strategy templates, reproducible reports, coverage-aware results, separate perp-project and listing studies, and additional on-chain, unlock, ETF and derivatives feeds as verified access permits.
