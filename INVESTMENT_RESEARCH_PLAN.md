...
**Progress — 2026-09-09:** stage 0 and the Stage 1 software foundation are complete. Stage 1 now includes persistent research state, Data Health with individual-series coverage, shared quotas for every acquisition path, normalized daily histories, selected BTC order-book/funding history, and sampled spot enrichment. The worker has 120 scheduled job definitions; empty enrichment slots skip without consuming quota or claiming coverage. Local Mac worker/backup supervision is installed and a database restore has been verified. **The remaining Stage 1 operational condition is an always-on host with sustained coverage; local supervision cannot guarantee uptime while this Mac sleeps or Docker is stopped.** See the [capability manifest](docs/data/CAPABILITY_MANIFEST.md), [quota budget](docs/data/quota-budget.json), [worker runbook](docs/data/DISCOVERY_WORKER.md), and [operation/restore procedures](ops/README.md). The next product implementation is Stage 2: BTC and the signal-study primitive. Manual backfills do not start replay coverage; the 1,000-asset expansion, macro and unsupported feeds remain gated.
...
Validation passes 78 checks: 35 API unit cases, 28 PostgreSQL integration cases, twelve capability/budget cases and three browser scenarios. API tests discover files recursively and assert minimum executed case counts with no skips; database and browser tests use isolated schemas. Coverage includes migrations, immutable revisions/replay, shared weighted quotas including manual/probe requests, lease/retry recovery, lifecycle and gap semantics, scope/null handling, persistent research state, note conflicts, threshold crossings with immutable evidence, and zero provider requests across API reads. Browser scenarios verify persistence after reload and Data Health rendering. Full backtest invariants remain future coverage.
