...
**Progress — 2026-09-09:** stage 0 and the Stage 1 software foundation are complete. Stage 2's BTC core and signal-study slice is now implemented: a quota-controlled Coin Metrics daily batch, long-term uPlot charts, daily/weekly indicators, confirmed regimes, valuation and drawdown/cycle comparisons, and immutable forward-return studies. The worker has 121 scheduled jobs and 20 normalized series; empty enrichment slots skip without consuming quota or claiming coverage. Live BTC price coverage is 2010-07-18–2026-09-09 (5,898 values); MVRV, market cap and supply currently end a day earlier and degrade individually. See [Stage 2 evidence](docs/data/stage2-btc-2026-09-09.json) and [methods](docs/data/BTC_RESEARCH.md). **The remaining Stage 1 operational condition is an always-on host with sustained coverage; local supervision cannot guarantee uptime while this Mac sleeps or Docker is stopped.** Local worker/backup supervision and restore verification remain in place. Next: Stage 2 venue-leverage/capital context, then Stage 3 Market Overview. Manual backfills do not start replay coverage; the 1,000-asset expansion, macro and unsupported feeds remain gated.
...
Validation passes 93 checks: 44 API unit cases, 32 PostgreSQL integration cases, twelve capability/budget cases and five browser scenarios. API tests discover files recursively and assert minimum executed case counts with no skips; database and browser tests use isolated schemas. Coverage includes migrations, immutable revisions/replay, shared weighted quotas including manual/probe requests, lease/retry recovery, lifecycle and gap semantics, scope/null handling, persistent research state, note conflicts, threshold crossings with immutable evidence, and stored-only API reads. Stage 2 adds regime confirmation and gap resets, completed weekly calculations, exact calendar outcomes, BTC-relative compounding, missing/immature horizons, immutable study reproducibility, chart controls/exports and reopening saved studies. Full strategy-backtest invariants remain future coverage.
...
- [ ] Stage 2 remaining context: surface the archived native BTC venue-leverage evidence and add covered stablecoin capital context after its individual feed/methodology checks. Mining inputs remain optional until their units and derivations are specified. Statistical holdouts, strategy costs/execution and the full Backtest Lab remain later work.
- [ ] Stages 3–6: deliver the remaining interfaces and research tools in the sequence above, subject to the recorded feed gates.
