...
const jobs=[...snapshotJobs.filter(j=>[marketDataset,globalDataset].includes(j.id)),historyJobs[0]];
...
for(const job of jobs){
  ...
  const payload=job.kind==='history'?{prices,total_volumes:prices}:job.id===marketDataset?[['bitcoin','BTC','Bitcoin',100],['ethereum','ETH','Ethereum',50]].map(([id,symbol,name,cap],index)=>({
