...
export async function storedTokenomicsEvents(assetId: string) {
  const result = await pool.query(`select id, event_type, amount, unit, effective_at, published_at, source_id, coverage, verification
